...
int main() {
    ...
    //SmartData
    Pressure_Keller_Capacitive level(0, 0);
    Water_Turbidity_OBS turb1(0, 0);
    Water_Turbidity_OBS turb2(1, 0);

    //Threads config
    new Periodic_Thread(INTEREST_PERIOD, &tstp_work, &level, &turb1, &turb2);
    new Periodic_Thread(HTTP_SEND_PERIOD, &http_send, &level, &turb1, &turb2);
