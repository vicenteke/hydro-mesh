...
// PWM
    ...
    static void config_SSI(volatile Log_Addr * base, Reg32 clock, SSI_Frame_Format protocol, SSI_Mode mode, Reg32 bit_rate, Reg32 data_width) {
        ...
        if(base == reinterpret_cast<Log_Addr *>(SSI0_BASE)) {
            ...
            if (Traits<A2035>::enabled) {
                if(mode == MASTER)
                    ioc(PA0_SEL) = SSI0_CLK_OUT;
                else
                    ioc(CLK_SSI_SSI0) = 0x00; //PA0

                ioc(PA6_SEL) = SSI0_FSS_OUT;
                ioc(PB5_SEL) = SSI0_TXD;
                ioc(SSIRXD_SSI0) = 0x17; //PC7
            } else { // eMote3 default pins
                if(mode == MASTER)
        ...
        if (Traits<A2035>::enabled) {
            ...
            //Set GPIO pins A0, A6, B5 and B7 to peripheral mode
            gpioa(AFSEL) |= (PIN0) + (PIN6);
            gpiob(AFSEL) |= (PIN5);
            gpioc(AFSEL) |= (PIN7);
        } else { // eMote3 default pins
            //To enable IO pads to drive outputs, the corresponding IPxx_OVER bits in IOC_Pxx_OVER register
