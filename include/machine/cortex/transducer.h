...
class ADC_Sensor_Test
{
public:
    static const unsigned int MAX_CHANNELS = 8;
    static const unsigned int PERIOD = 1000; //50 ms
...
private:
    typedef Simple_Hash<Data, MAX_CHANNELS, unsigned int> Readings;

public:
    ADC_Sensor_Test() {}

    static void attach(Observer * obs, unsigned int ch, unsigned int period, unsigned int thresh) {
        assert(_readings.size() < MAX_CHANNELS);
        // OStream cout; cout << "Attach: " << ch << endl;
        Readings::Element * _link = _readings.search_key(ch);
        if(_link)
            _link->object()->obs->attach(obs);
        else {
            Data * data = new Data();
            data->obs = new Observed();
            data->obs->attach(obs);
            data->adc = new ADC(static_cast<ADC::Channel>(ch));
            data->period = period;
            data->thresh = thresh;
            data->last_value = 0;
            data->last_time = 0;
            _keys[_readings.size()] = ch;
            _link = new Readings::Element(data,ch);
            _readings.insert(_link);
        }
        if(!_thread) {
            // cout << "New Thread" << endl;
            _thread = new Thread(&loop);
        }
        // cout << "readings_size=" << _readings.size() << endl;
    }
    static void detach(Observer * obs, unsigned int ch) {
        Readings::Element * _link = _readings.search_key(ch);
        if(_link)
            _link->object()->obs->detach(obs);
    }
    static void notify(unsigned int ch) { 
        Readings::Element * _link = _readings.search_key(ch);
        if(_link)
            _link->object()->obs->notify();
    }
private:
    static int loop() {
        OStream cout;
        while(true) {
            // Alarm::delay(PERIOD);
            // for(Readings::Iterator it = _readings.begin(); it != _readings.end(); it++) {
            //     // Readings::Element * _link = *it;
            //     cout << "adc " << (*it).key() << endl;
            //     int value = it->object()->adc->read();
            //     int last_value = it->object()->last_value;
            //     int diff = (last_value < value) ? (value - last_value) : (last_value - value);
            //     TSTP::Time last_time = it->object()->last_time;
            //     TSTP::Time t = TSTP::now();
            //     if((diff >= it->object()->thresh) || ((t - last_time) >= it->object()->period)) {
            //         it->object()->last_value = value;
            //         it->object()->last_time = t;
            //         it->object()->obs->notify();
            //     }
            // }

            for(unsigned int i = 0; i < _readings.size(); i++) {
                Readings::Element * _link = _readings.search_key(_keys[i]);
                int value = _link->object()->adc->read();
                // cout << "adc " << _keys[i] << "=" << value << endl;
                int last_value = _link->object()->last_value;
                int diff = (last_value < value) ? (value - last_value) : (last_value - value);
                TSTP::Time last_time = _link->object()->last_time;
                TSTP::Time t = TSTP::now();
                if(((diff >= _link->object()->thresh) && (_link->object()->thresh !=0)) || ((t - last_time) >= _link->object()->period)) {
                    _link->object()->last_value = value;
                    _link->object()->last_time = t;
                    _link->object()->obs->notify();
                }
            }
        }
...
private:
    ...
    static Readings _readings;
    static unsigned int _keys[MAX_CHANNELS];
};

template<unsigned int _CH, unsigned int _P, unsigned int _T, unsigned int _U>
class ADC_Wrapper : private ADC_Sensor_Test
{
public:
    static const unsigned int UNIT = _U; // I32 by default
    static const int ERROR = 0; // Unknown
...
public:
    static void attach(Observer * obs) { ADC_Sensor_Test::attach(obs, _CH, _P, _T); }
    static void detach(Observer * obs) { ADC_Sensor_Test::detach(obs, _CH); }

    template<typename SD>
    static void sense(unsigned int dev, SD * data) {
        Readings::Element * _link = _readings.search_key(_CH);
        data->_value = _link->object()->last_value;
    }
...
class Iluminance_Sensor
{
public:
    static const unsigned int UNIT = TSTP::Unit::Luminous_Intensity; // I32 by defaut
    static const int ERROR = 0; // Unknown
...
public:
    ...
    static void sense(unsigned int dev, Smart_Data<Iluminance_Sensor> * data) {
        if(!_opt3001) 
            _opt3001 = new OPT3001();
        data->_value = _opt3001->get();       
    }

    static void actuate(unsigned int dev, Smart_Data<Iluminance_Sensor> * data, const Smart_Data<Iluminance_Sensor>::Value & command) {}
...
public:
    ...
    static void sense(unsigned int dev, Smart_Data<I2C_Temperature_Transducer> * data) {
        if(!sensor){
            sensor = new I2C_Temperature_Sensor();
        }
...
class Accelerometer_LSM330
{
private:
    static const unsigned int BATCH_SIZE = 5; // 5 * (offset + x + y + z)
    static const unsigned int BUFFER_SIZE = 150; // 150 * ( 5 * (offset+x+y+z) )
    static const unsigned int PERIOD = 50000; // 2s
    static const unsigned int SD_PERIOD = 1000000; // 2s
    ...
    static int analyzer(){
        ...
        while(true) {
            ...
            if((t - last_t) >= PERIOD) {
                ...
                data->m[index].x = _imu->acc_x;
                data->m[index].y = _imu->acc_y;
                data->m[index].z = _imu->acc_z;
...
class Accelerometer_LSM330_Wrapper : private Accelerometer_LSM330
{
public:
    static const unsigned int UNIT = TSTP::Unit::Digital_Unit<((1<<15)-1),sizeof(Accelerometer_LSM330::Data)>::UNIT;
    static const int ERROR = 0;
...
public:
    static void attach(Observer * obs) { Accelerometer_LSM330::attach(obs); }
    static void detach(Observer * obs) { Accelerometer_LSM330::detach(obs); }
...
typedef Smart_Data<Accelerometer_LSM330_Wrapper> Acceleration_LSM330;
...
typedef Smart_Data<Iluminance_Sensor> Iluminance;
