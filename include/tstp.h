...
public:
    ...
    // 32-bits specialization (no padding) below
    template<Scale S>
    struct _Padded_Coordinates: public _Coordinates<S>
    {
        _Padded_Coordinates(const _Coordinates<S> & c) : _Coordinates<S>(c) {}
    private:
        typename _Coordinates<S>::Number _padding;
    }__attribute__((packed));
    typedef _Padded_Coordinates<SCALE> Padded_Coordinates;
    ...
    template<Scale S>
    struct _Region: public Time_Interval, public Sphere<typename _Coordinates<S>::Number>
    {
        typedef typename _Coordinates<S>::Number Number;
        typedef Sphere<Number> Space;
        typedef typename Space::Center Center;

        _Region(const Center & c, const Number & r, const Time & t0, const Time & t1)
        : Time_Interval(t0, t1), Space(c, r) {}

        bool contains(const Center & c, const Time & t) const {
            return Time_Interval::contains(t) && Space::contains(c);
        }

        friend Debug & operator<<(Debug & db, const _Region & r) {
            db << "{" << static_cast<const Space &>(r) << ",t0=" << r.t0 << ",t1=" << r.t1 << "}";
            return db;
        }
    ...
    // Packet Header
    // In every TSTP message, all types greater than 32bits must be aligned at 32bits
    template<Scale S>
    class _Header
    {
        // Format
    ...
    public:
        ...
        : _config((S & 0x03) << 6 | tr << 5 | (t & 0x03) << 3 | (v & 0x07)), _confidence(c), _subtype(st), _time(ot), _origin(o), _last_hop(l) {}
        ...
        unsigned short subtype() const { return _subtype; }
        void subtype(unsigned short st) { _subtype = st; }

        friend Debug & operator<<(Debug & db, const _Header & h) {
            db << "{v=" << h.version() - V0 << ",t=" << ((h.type() == INTEREST) ? 'I' :  (h.type() == RESPONSE) ? 'R' : (h.type() == COMMAND) ? 'C' : 'T') << ",co=" << h._confidence << ",st=" << h._subtype << ",tr=" << h.time_request() << ",s=" << h.scale() << ",ot=" << h._time << ",o=" << h._origin << ",lt=" << h._last_hop_time << ",l=" << h._last_hop << "}";
            return db;
        }
    ...
    protected:
        ...
        Time _time;
        Time _last_hop_time;
        _Padded_Coordinates<S> _origin;
        _Padded_Coordinates<S> _last_hop;
    } __attribute__((packed));
    ...
    protected:
        ...
        : Header(CONTROL, st, tr, c, ot, o, l, v) {}
    } __attribute__((packed));
    ...
    public:
        ...
        enum Quantity {
            ...
            Frequency               = SI_Unit<I32, DIR,     +0,     +0,     +0,     +0,     -1,     +0,     +0,     +0,     +0>::UNIT,
            ...
            Energy                  = SI_Unit<I32, DIR,     +0,     +0,     +2,     +1,     -2,     +0,     +0,     +0,     +0>::UNIT,
            Voltage                 = SI_Unit<I32, DIR,     +0,     +0,     +2,     +1,     -3,     -1,     +0,     +0,     +0>::UNIT,
            Power                   = SI_Unit<I32, DIR,     +0,     +0,     +2,     +1,     -3,     +0,     +0,     +0,     +0>::UNIT,
            Irradiance              = SI_Unit<I32, DIR,     +0,     +0,     +0,     +1,     -3,     +0,     +0,     +0,     +0>::UNIT,
        };
    ...
    // TODO
    static Frame_ID id(Frame * f) {
        Frame_ID ret = 0;
        Frame_ID * aux = reinterpret_cast<Frame_ID *>(f);
...
template<> struct TSTP_Common::_Padded_Coordinates<TSTP_Common::Scale::CM_32>: public _Coordinates<TSTP_Common::Scale::CM_32>
{
...
public:
    ...
    {
    protected:
        typedef unsigned char Data[MTU - 4*sizeof(unsigned char) - sizeof(Region) - sizeof(Unit) - 2*sizeof(Time) - sizeof(CRC)];
    public:
        ...
        : Header(INTEREST, Subtype::NONE, 0, 0, now(), here(), here()), _mode(mode), _precision(0), _predictor(Predictor_Common::NONE), _region(region), _unit(unit), _expiry(expiry), _period(period) {}

        template<typename C=unsigned int>
        Interest(const Region & region, const Unit & unit, const Mode & mode, const Error & precision, const Microsecond & expiry, const Microsecond & period, const Predictor & predictor, const C & configuration = 0x00)
        : Header(INTEREST, Subtype::NONE, 0, 0, now(), here(), here()), _mode(mode), _precision(0), _predictor(predictor), _region(region), _unit(unit), _expiry(expiry), _period(period)
        {
            memset(_predictor_config, 0, sizeof(Data));
            *predictor_config<C>() = configuration;
        }
        ...
        template<typename T>
        T * predictor_config() { assert(has_config()); return reinterpret_cast<T *>(&_predictor_config); }

        friend Debug & operator<<(Debug & db, const Interest & m) {
            db << reinterpret_cast<const Header &>(m) << ",m=" << ((m._mode == ALL) ? 'A' : 'S') << ",e=" << int(m._precision) << ",pr=" << m._predictor << ",hm=" << m._has_model << ",re=" << m._region << ",u=" << m._unit << ",x=" << m._expiry << ",p=" << m._period << ",pc=" << hex << *const_cast<Interest &>(m).predictor_config<unsigned>() << dec;
            return db;
        }
    ...
    protected:
        unsigned char _mode;
        char _precision;
        unsigned char _predictor;
        unsigned char _has_model;
        Region _region;
        Unit _unit;
        Time _expiry;
        Time _period;
    ...
    public:
        ...
        : Header(COMMAND, Subtype::NONE, 0, 0, now(), here(), here()), _region(region), _unit(unit) {}
        ...
        template<typename T>
        T * data() { return command<T>(); }

        friend Debug & operator<<(Debug & db, const Command & m) {
            db << reinterpret_cast<const Header &>(m) << ",u=" << m._unit << ",reg=" << m._region << ",d=" << hex << *const_cast<Command &>(m).data<unsigned>() << dec;
            return db;
        }
    ...
    public:
        ...
        : Control(DH_REQUEST, 0, 0, now(), here(), here()), _public_key(k), _destination(dst) { }
    ...
    public:
        ...
        : Control(AUTH_GRANTED, 0, 0, now(), here(), here()), _auth(a), _destination(dst) { }
    ...
    public:
        ...
        friend Debug & operator<<(Debug & db, const Epoch & e) {
            db << reinterpret_cast<const Control &>(e) << ",d=" << e._destination << ",e=" << e._epoch << ",c=" << e._coordinates;
            return db;
        }
    ...
    public:
        ...
        void command(const Value & v) {
            ...
            Region command_region = region();
            command_region.t0 = TSTP::now();
            command_region.t1 = command_region.t0 + expiry();
            Command * command = new (buf->frame()->data<Command>()) Command(unit(), command_region);
        ...
        template<typename C>
        void predictor_update(const C & configuration, Region * _region = 0) {
            unsigned int size = sizeof(Interest) - sizeof(Interest::Data) + sizeof(C);
            Buffer * buf = alloc(size);
            new (buf->frame()->data<Interest>()) Interest(_region?*_region:region(), unit(), mode(), precision(), expiry(), period(), predictor(), configuration);
            marshal(buf);
            _nic->send(buf);
        }
    ...
    public:
        ...
        void interest(const Interest & interest) {
            if(_interest)
                delete _interest;
            _interest = new Interest(interest);
        }
        const Interest * interest() const { return _interest; }

        void model_listener(const bool & b) { _model_listener = b; }
        bool model_listener() const { return _model_listener; }
    ...
    class Timekeeper: private NIC::Observer
    {
        ...
        static const bool disable_time_request = false;
    ...
    public:
        ...
        static bool synchronized() { return !sync_recommended(); }

        static bool sync_required() {
            return (!disable_time_request) && (now() >= _next_sync);
        }

        static bool sync_recommended() {
            return (_next_sync == 0) || (now() >= (_next_sync - SYNC_PERIOD / 2));
        }
    ...
    private:
        static void keep_alive() { TSTP::keep_alive(); }
    ...
    public:
        Security() {
            ...
            new (&_id) Node_ID(Machine::id(), sizeof(_SYS::UUID));
    ...
    private:
        ...
        static OTP otp(const Master_Secret & master_secret, const Node_ID & id) {
            ...
            OTP out;
            Poly(id, ms).stamp(out, nonce, mi, MI_SIZE);
            return out;
        }
...
private:
    ...
    static void keep_alive(bool unset_time_request = false) {
        db<TSTP>(TRC) << "TSTP::keep_alive(" << unset_time_request << ")" << endl;
        Buffer * buf = alloc(sizeof(Keep_Alive));
        Keep_Alive * keep_alive = new (buf->frame()->data<Keep_Alive>()) Keep_Alive;
        marshal(buf);
        buf->deadline = now() + Timekeeper::SYNC_PERIOD;
