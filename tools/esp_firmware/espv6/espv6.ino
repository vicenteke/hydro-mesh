...
void act(){
    ...
    if(strncmp(action,"SETAUTHMETHOD",action_size) == 0){
        ...
        bool ssid_done = false;
        bool pass_done = false;
        bool username_done = false;
        int count = 0;
        for(int i=0;i<data_size;i++) {
            if((data[i] == ',') || (i==data_size-1) ) {
                char temp[SSID_MAX_SIZE]={};
                if(!ssid_done) {
                    memcpy(temp,data,i);
                    if(!(iot->set_ssid(temp,i)))
                        return;
                    ssid_done=true;
                    temp[i]='\0';
                }
                else if(!pass_done && ssid_done) {
                    int temp_size = i-count;
                    if(data[i]==',')
                        temp_size = temp_size-1;
                    memcpy(temp,&data[count+1],temp_size);
                    if(!(iot->set_pass(temp,temp_size)))
                        return;
                    pass_done=true;
                    temp[temp_size]='\0';
                }
                else if(!username_done && ssid_done && pass_done) {
                    memcpy(temp,&data[count+1],i-count);
                    if(!(iot->set_username(temp,i-count)))
                        return;
                    iot->set_auth_method(ENTERPRISE);
                    username_done=true;
                    temp[i-count]='\0';
                }
                count=i;
...
//        iot->get_ip();
    ...
    } else if(strncmp(action,"APIATTACH",action_size) == 0){
        if(data_size == 0) {
            Serial.print("ERR");
        }
        iot->set_route(IOTATTACH, strlen(IOTATTACH));
        int res = iot->http_post((unsigned char *) data, data_size);

        Serial.print("+HTTPCODE=");
        Serial.print(res);

    } else if(strncmp(action,"APIPUT",action_size) == 0){
        if(data_size == 0) {
            Serial.print("ERR");
        }
        iot->set_route(IOTPUT, strlen(IOTPUT));
        int res = iot->http_post((unsigned char *) data, data_size);
